...
class Terminal(Controller):
    ...
    def set(self,value):
        ...
        if not self.tn:
            print self.LOST_CONNECT
            return (False, "LOST_CONNECTION")

        if self.log&1:
            print "[SET]:",value
        try:
            self._set(value+self.cr)
            return (True, "KEEP_CONNECTION")
        except:
            self.tn = 0
            print self.LOST_CONNECT
            return (False, "LOST_CONNECTION")
    ...
    def wait(self,prompt,timeout):
        ...
        while time.time() < timeout and self.tn:
            ...
            response += self.get(); #print response
            if not prompt:
                return (False, response)
            else:
                if prompt in response:
                    #if count >= 3:  print
                    return (True, 'htx_local:'+response)
            #time.sleep(0.1)
        #if count >= 3: print
        if self.log&1:
            print "Terminal: Timeout"
        return (False, self.tn, response)
