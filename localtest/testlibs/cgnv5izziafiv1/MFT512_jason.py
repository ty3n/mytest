...
def WaitingAndCheckBoot_telnet(*argv):
    '''
     argv :
    ...
    for try_ in range(3):
        ...
        time.sleep(1)           
        '''
        lWaitCmdTerm(term,'top','mainMenu>',5,2)  
        lWaitCmdTerm(term,'docsis','docsis>',5,2)      
        #lWaitCmdTerm(term,'telnettimeout','Input >',5)
        #lWaitCmdTerm(term,'30','is>',5,3) 
        #term<<"\n" 
        for i in range(3):
            section = lWaitCmdTerm(term,'dir','docsis>',5,2)
            #if section.count(customer) == 3:
            if ("Filename in sector 1->%s"%customer1 in section) or ("Filename in sector 2->%s"%customer2 in section):
               argv[4](section)
               break               
            else:
        ...
        if 'Selected sector is 2' in section:           
           argv[4]('Selected sector 2 check pass',2)            
        else:
           lWaitCmdTerm(argv[1][-1],'bootfrom 2','is>',5,3)
...
def ResetButton(*argv):
    ...
    for p in range(20):
        time.sleep(3)
        data = lWaitCmdTerm(argv[1][2],'ping 1 192.168.1.1 -n 1','ping',8,2)
...
def WaitResetboot_telnet(*argv):
    ...
    for p in range(6):
        ...
        if p==4 :raise Except('ErrorCode(E00155):Reset boot failed') 
    argv[-4]('Waiting DUT up pass',2) 


def CmtsLockCheck(term,CMTS_freq,Tuner_Menu):
    data = lWaitCmdTerm(term,"phy 0","docsis>",15,3)
    data=data.split("QAM Lock:")[-1].split("Carrier offset")[0].count("YES")
    if data  == 3: 
       return 1
    else:
       lWaitCmdTerm(term,"tune %s"%CMTS_freq,"docsis>",8,2)
       time.sleep(10)
    return 0  

def WaitCMRegistration_20170901(*argv):
    '''
     argv :
         dutid,terms,labels,Panel,Log,Config,flow,[Return])
         terms : ccu , cb , sw , vm ,dut 
    ...
    if port >4 : port -= 4
    dhcp=0
    lWaitCmdTerm(cbterm,'rf %s c'%port,'rf',15,3)
    etime=time.time()+timeout
    t = time.time() #by James_Huang 2015/12/04
    while time.time()<= etime:         
         data = lWaitCmdTerm(term,"cmstatus","is>",5,5)
         #argv[-4](data)
         print data
         if "OPERATIONAL" in data: 
            argv[-4]('CM Operational OK ',2)
            return 
         if 'DHCPV4_IN_PROGRESS' in data:
             dhcp+=1
         if 'NOT_SYNCHRONIZED' in data or dhcp > 10:    
            CmtsLockCheck(term,CMTS_freq,Tuner_Menu)
            dhcp=0
         time.sleep(3)     
    raise Except("ErrorCode(E00125):Registration Timeout.--%s"%data)  
    #raise Except("ErrorCode(E00125):Registration %3.2f sec Timeout. [ %s ]"%((time.time()-t),data)) 

def WaitCMRegistration_20170905(*argv):
    '''
     argv :
         dutid,terms,labels,Panel,Log,Config,flow,[Return])
         terms : ccu , cb , sw , vm ,dut 
    ...
    while time.time()<= etime:
         #lWaitCmdTerm(cbterm,'rf %s c'%port,'rf',5,3)
         data = lWaitCmdTerm(term,"cmstatus","is>",8,2)
         print data
         if "OPERATIONAL" in data: 
            argv[-4]('CM Operational OK ',2)
            return 
         if 'DHCPV4_IN_PROGRESS' in data:
             ...
             term<<"quit"
             time.sleep(35)
             argv[1][-1] = lLogin(dstip,pid,username,password_)
             term = argv[1][-1]
             lWaitCmdTerm(term,"docsis","is>",5,2)
         if 'NOT_SYNCHRONIZED' in data or dhcp > 10:    
            CmtsLockCheck(term,CMTS_freq,Tuner_Menu)
            dhcp=0
         time.sleep(3)     
    raise Except("ErrorCode(E00125):Registration Timeout.")     
...
def WaitCMRegistration(*argv):
    ...
    while time.time()<= etime:
         lWaitCmdTerm(cbterm,'rf %s c'%port,'rf',5,3)
         data = lWaitCmdTerm(argv[1][-1],"cmstatus","is>",5,5)
         #argv[-4](data)
         if "OPERATIONAL" in data: 
            #lWaitCmdTerm(argv[1][-1],"De","g>",5,5)
            #data = lWaitCmdTerm(argv[1][-1],"ds","g>",5,5)
            #argv[-4](data,2)
            argv[-4]('\nCM Operational OK\n',2)
            return 
         if 'RANGING_COMPLETE' in data:
            ...
            while 1:
              if etime>time.time():                
                #term = htx.Telnet(ETH0IP_,pid)
                argv[1][-1] = lLogin(ETH0IP_,pid,username,password_)
                print 'Telnet open OK!'
                lWaitCmdTerm(argv[1][-1],"doc","is>",5,5)
                break
              if etime<time.time(): raise Except("ErrorCode(E00125):telnet failed.")  
              time.sleep(2)
...
def Check24X8UsDsLock(*argv):
    ...
    try:
        for k in range(3):
            ...
            if data.count('YES') == 96:break
            if k == 2:raise Except("ErrorCode(E00242):24 DS Channel Lock Check: FAIL")
            time.sleep(1)             
        log("24 DS Channel Lock Check: PASS (PASS)",2)
        ...
        for k in range(3):
            ...
            for i in US:           
              Ds_n += int(i)
            if Ds_n != 36:
               if k==2:raise Except("ErrorCode(E00242):8 US Channel Lock Check: FAIL")
            else:break 
        log("8 US Channel Lock Check: PASS (PASS)",2)
    except:
    ...
    for try_ in range(3):
        try:
            ...
            for j in range(8):
                ...
                if us_data[j][1] <> "ERR":
                     if us_data[j][0]>15 and us_data[j][0]<40:
                         count+=1
                         ...
                         msg = "US Freq = %d MHz, DUT_uspower = %.2f,CMTS_uspower = %.2f, diff = %.2f(%.2f ~ %.2f)"%(us_data[j][0], us_data[j][2], system_uspower, diff, uspower_offset*-1,uspower_offset)
...
def InformationCheck(*argv):
    ...
    lWaitCmdTerm(term,"exit","sis>",5)
    data=lWaitCmdTerm(term,"ven","sis>",5)
    if 'Error: Command not found.' in data:
       lWaitCmdTerm(term,"top",menu,5)
    for try_ in range(3):
...
def USBTest(*argv):
    ...
    if not os.path.isfile('%s\\usbtest.dat'%openssl_path):
       os.system('copy %s\\usbtest.dat %s\\'%(mta_cer_path,openssl_path))
    if not os.path.isfile('%s\\usbtest.dat'%openssl_path):
       raise Except('No such file usbtest.dat')
    for i in range(3):
        mount_data = lWaitCmdTerm(term,"mount","#",5)
        print mount_data
        print "/dev/sda1 on %s"%usb_disk
        print mount_data.index("/dev/sda1 on %s"%usb_disk)
        if "/dev/sda1 on %s"%usb_disk in mount_data:
           if 'usbtest.dat' not in lWaitCmdTerm(term,"ls %s"%usb_disk,"",5):
              lWaitCmdTerm(term,"tftp -g -r usbtest.dat -l %s/usbtest.dat %s"%(usb_disk,tftp_ip),"",5)
           lWaitCmdTerm(term,"cp %s/usbtest.dat /var/tmp"%usb_disk,"",5)
        elif "/dev/sda on %s"%usb_disk[:-1] in mount_data:
            if 'usbtest.dat' not in lWaitCmdTerm(term,"ls %s"%usb_disk[:-1],"",5):
              lWaitCmdTerm(term,"tftp -g -r usbtest.dat -l %s/usbtest.dat %s"%(usb_disk[:-1],tftp_ip),"",5)
            lWaitCmdTerm(term,"cp %s/usbtest.dat /var/tmp"%usb_disk[:-1],"",5)
        else:
            if i==2:raise Except ("Please Check the USB device.")
            time.sleep(8)
        data=lWaitCmdTerm(term,"ls /var/tmp","#",5)
        print data
        if not "usbtest.dat" in data:
           if i==2:raise Except ("ErrorCode(E00146):USB Test failed")
        else:break
    log("USB TEST PASS",2)
...
#parameter=dict()
telnet_locks=thread.allocate_lock()
def TelnetLogin_select(*argv):
    ...
    if argv[1][-1]:
      ...
      if telnet_locks.locked: telnet_locks.release()
