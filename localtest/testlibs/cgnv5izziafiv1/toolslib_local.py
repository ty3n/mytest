import decimal
import time,thread,htx_local,odbc,pyodbc,os
...
config_path=os.getcwd()+'\\cmd_path.ini'
...
def lWaitCmdTerm_(term,cmd,waitstr,sec,count=1):
    termport=term.host
    termdata=['']
...
def lWaitCmdTerm(term,cmd,waitstr,sec,count=1):    
    ...
    if not term_locks.has_key(termport): term_locks[termport]=thread.allocate_lock()  
    term_locks[termport].acquire() 
    for try_ in xrange(count):
      if '192' in term.host:        
        term=temp.pid_parameter[termport][1][-1]
      termdata=list()
      try:
        term << cmd
      except: pass
      termdata=term.wait(waitstr,sec); print (termport,':',termdata)
      #raw_input(waitstr)
      if waitstr in termdata[-1] and 'ttyS0: 1 input overrun' not in termdata[-1]:
        if term_locks[termport].locked: term_locks[termport].release()
        return termdata[-1]
      elif 'LOST_CONNECTION' in termdata[-1]:
        if '192' in term.host:
          temp.pid_parameter[termport][-4]('session off, reconnect..("%s port : %s  return : %s")'%(cmd,termport,termdata[-1]))
          import MFT512
          argv=temp.pid_parameter[termport]
          if term_locks[termport].locked: term_locks[termport].release()
          term=eval('MFT512.TelnetLogin_select(argv[0],argv[1],argv[2],argv[3],argv[4],argv[5],argv[6],argv[7])')
          if term:
            term_locks[termport].acquire() 
            for path in cmd_path[waitstr]:
              print cmd; time.sleep(0.5)
              for retry_ in xrange(3):
                 termdata=list()
                 term<<path[0]
                 termdata=term.wait(path[1],path[2]); print(termport,'<reconnect>:',termdata)
                 if waitstr in termdata[-1] and 'ttyS0: 1 input overrun' not in termdata[-1]:
                    if term_locks[termport].locked: term_locks[termport].release()
                    return termdata[-1]
      else: pass
    ...
    if '192' in term.host: term.close()
    raise Except("%s port : %s  return : %s"%(cmd,termport,termdata[-1]))

def TelnetWaitCmdTerm(term,cmd,waitstr,sec,count=1):
    if term.host.split('.')[0] == '192': termport = '%s-%s'%(term.host,term.port) 
    else: termport=term.host 
    termdata=['']
    if not term_locks.has_key(termport):
    ...
    for i in range(count):
       ...
       term << "%s"%cmd
       termdata = term.wait("%s"%waitstr,sec); print termdata
       if waitstr in termdata[-1] or '101' in termdata[-1]:
          if term_locks[termport].locked: 
             term_locks[termport].release()
          return termdata[-1]
    if term_locks[termport].locked: 
...
def CheckHWandSWfromDB(*argv):
    ...
    hsw = map(strip,argv[-3]('Base','HSW').split('|'))
    if len(hsw)== 3:
       hw,sw,mfg = hsw
    else:
       hw,sw = hsw
    ...
    db = odbc.odbc("TESTlog/TEST/test")
    cursor = db.cursor()
    sql="SELECT HW,SW,MFGSW FROM VersionControl WHERE PN='%s' ORDER BY DATATIME DESC"%pn
    cursor.execute(sql)
    data = cursor.fetchone()
    if not data:raise Except("ErrorCode(0011):get hw and sw failed from db")
    if not data[-1]:mfg_db=""
    else:mfg_db = data[-1]
    #print hw ,data[0], sw, data[1],mfg, mfg_db
    if hw <> data[0] or sw <> data[1] or mfg <> mfg_db: 
